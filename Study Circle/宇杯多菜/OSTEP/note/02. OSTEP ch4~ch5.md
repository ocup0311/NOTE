...
- 杯：
  ...
  - 而`wait()`只能等待最新一個 child <mark>TODO:Q</mark> 不知為何查到的資料跟[文件](https://man7.org/linux/man-pages/man2/wait.2.html)上都是說`wait()`會等待任何一個 child 結束，即可回傳。但我的測試卻是等待最新一個 child 結束才回傳？
